        ...
        >
          {items.map((article) => (
            <li key={article.slug}>
              <Link
                href={article.href}
                className="group flex gap-4 overflow-hidden rounded-[var(--radius-card)] border border-[var(--line-soft)] bg-white/90 p-3 pr-4 shadow-[var(--shadow-soft)] transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-0.5 hover:border-[var(--ink)]/10 hover:shadow-[var(--shadow-card)] sm:gap-5 sm:p-4"
              >
                <div
                ...
                <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
                  ...
                  <p className="font-serif text-sm leading-snug text-[var(--ink-muted)] sm:text-[0.95rem]">
                    {article.dek}
                  </p>
                  <span className="mt-1 inline-flex items-center gap-1 font-sans text-xs font-medium text-[var(--ink-muted)] transition-colors group-hover:text-[var(--ink)]">
                    Open
                    <span
